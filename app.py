...
def add_like(message_id):
    ...
    user_now = User.query.get_or_404(g.user.id)
    message_ids = [u.id for u in user_now.likes]
    if message_id in message_ids:
        prev_like = Likes.query.filter_by(message_id=message_id).all()
        db.session.delete(prev_like[0])
        db.session.commit() 
        return redirect("/")   
...
def homepage():
    ...
    if g.user:
        ...
        messages = [item for message in nested_messages for item in message]
